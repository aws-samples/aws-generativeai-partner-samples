...
class OpenSearchServerlessInfraStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs)-> None:
        ...
        ssm.StringParameter(self, 'collectionArn', parameter_name="/e2e-rag/collectionArn", string_value=self.collection.attr_arn)
        
        # Store the OSS URL in SSM parameter for Splunk_helper to use
        ssm.StringParameter(self, 'ossUrl',
                            parameter_name="/e2e-rag/ossUrl",
                            string_value=self.collection.attr_collection_endpoint)

        self.create_oss_index()
        
        # Output the OSS URL
        CfnOutput(self, "OssUrl", 
                  value=self.collection.attr_collection_endpoint, 
                  description="OpenSearch Serverless URL endpoint")
    ...
    def create_data_access_policy(self) -> CfnAccessPolicy:
      ...
      lambda_role_arn = ssm.StringParameter.from_string_parameter_attributes(self, "LambdaRoleArn",
                        parameter_name="/e2e-rag/lambda-role").string_value
