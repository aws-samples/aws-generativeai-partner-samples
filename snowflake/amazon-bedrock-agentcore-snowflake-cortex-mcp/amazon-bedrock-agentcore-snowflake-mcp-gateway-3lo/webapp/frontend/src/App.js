  ...
  return (
    <div className="login-screen">
      <div className="login-box">
        ...
        <p>3LO — Hybrid RAG: Bedrock KB + Snowflake MCP Server (Per-User OAuth)</p>
...
function TracePanel({ trace }) {
  ...
  return (
    <div className="trace-panel">
      ...
      {toolCalls.map((tc, i) => {
        ...
        if (name === 'cortex_search') return (
          <div key={i} className="trace-item">
            ...
            <div className="detail">Cedar: ENFORCE ✅ | 3LO: Per-user OAuth ✅</div>
          </div>
        );
      ...
      {hasCortex && (
        <div className="trace-item"><div className="label">🔑 AgentCore Identity (3LO)</div><div className="detail">Per-user Snowflake OAuth ✅</div></div>
      )}
      <div className="trace-item">
...
export default function App() {
  ...
  // --- Handle /auth/snowflake-callback route ---
  useEffect(() => {
    const path = window.location.pathname;
    if (path !== '/auth/snowflake-callback') return;
    // Prevent double execution
    if (window._sfCallbackHandled) return;
    window._sfCallbackHandled = true;

    const params = new URLSearchParams(window.location.search);
    const sessionUri = params.get('session_id') || params.get('session_uri') || params.get('sessionUri') || params.get('sessionId');
    console.log('[3LO callback] search:', window.location.search, 'sessionUri:', sessionUri);

    if (!sessionUri) {
      console.error('[3LO callback] No session_id in callback URL');
    ...
    (async () => {
      try {
        ...
        if (!h) {
          console.error('[3LO callback] No Cognito session');
          window.location.href = '/';
          return;
        }
        const resp = await fetch(`${API}/auth/complete-snowflake-auth`, {
          method: 'POST', headers: h,
          body: JSON.stringify({ session_uri: sessionUri }),
        });
        if (resp.ok) {
          console.log('[3LO callback] CompleteResourceTokenAuth succeeded');
          ...
          console.error('[3LO callback] CompleteResourceTokenAuth failed:', resp.status, err);
        }
      } catch (e) {
        console.error('[3LO callback] Error:', e);
      }
      window.location.href = '/';
    })();
  }, []); // eslint-disable-line
  ...
  useEffect(() => {
    ...
    fetch(`${API}/auth/snowflake-status`, { headers: h }).then(r => r.json()).then(d => {
  ...
  // --- Connect to Snowflake (3LO) ---
  const connectSnowflake = async () => {
    setSfConnecting(true);
    try {
      const h = await getHeaders();
      if (!h) return;
      const resp = await fetch(`${API}/auth/snowflake-auth-url`, { headers: h });
      if (!resp.ok) throw new Error('Failed to get auth URL');
      const data = await resp.json();
      if (data.auth_url) {
        // Navigate to Snowflake login in the same tab
      ...
      console.error('Connect Snowflake failed:', e);
      alert('Failed to initiate Snowflake connection. Please try again.');
    } finally {
      ...
      if (data.job_id) {
        ...
        for (let i = 0; i < 100; i++) {
          ...
          try {
            ...
            if (pd.status === 'snowflake_auth_required') {
              localStorage.removeItem('sf_connected');
              localStorage.removeItem('sf_identity');
              setSfConnected(false);
              setSnowflakeIdentity(null);
              setMessages(prev => {
                ...
                msgs.push({ role: 'assistant', content: '🔐 Snowflake authentication required. Please click the "Connect to Snowflake" button in the header, then retry your question.' });
                return msgs;
              });
              return;
            }
    ...
    <div className="app-layout">
      <div className="header">
        <div className="header-left">
          ...
          <span className="tag">3LO — Hybrid RAG: KB + Snowflake MCP Server (Per-User OAuth)</span>
        </div>
        <div className="header-right">
          {sfConnected ? (
            <span className="sf-badge connected" title="Snowflake identity (per-user 3LO)">
              ❄️ Connected · {snowflakeIdentity ? `${snowflakeIdentity.user} / ${snowflakeIdentity.role}` : 'Snowflake'}
            </span>
          ) : (
            <button className="sf-connect-btn" onClick={connectSnowflake} disabled={sfConnecting}>
              {sfConnecting ? '⏳ Connecting...' : '❄️ Connect to Snowflake'}
            </button>
          )}
            ...
            {messages.length === 0 && (
              <div style={{ color: '#546e7a', textAlign: 'center', marginTop: 60, fontSize: 14 }}>
                {!sfConnected && <div style={{ marginBottom: 16, padding: '12px 16px', background: '#fff3e0', borderRadius: 8, display: 'inline-block' }}>
                  ❄️ <strong>Step 1:</strong> Click "Connect to Snowflake" in the header to authenticate with your Snowflake account.
                </div>}
