"""Credit Risk Agent Backend — FastAPI with Cognito JWT auth, async polling, 3LO support."""
import os
import json
import re
import time
...
app = FastAPI(title="Credit Risk Agent API (3LO)")
...
def _fetch_snowflake_identity(user_token: str) -> Optional[dict]:
    ...
    payload = json.dumps({
        ...
        "params": {
            "name": "SnowflakeMCPServer3LO___sql-exec",
    ...
    for t in texts:
        try:
            ...
            continue
        # Shape A: Snowflake sql-exec native — {"result_set":{"data":[["SKAMALAR","ANALYST_ROLE"]]}}
        ...
        # Shape B: list of dicts — [{"U": "SKAMALAR", "R": "ANALYST_ROLE"}]
...
# --- 3LO: Get Snowflake auth URL by triggering a Gateway call ---
@app.get("/api/auth/snowflake-status")
async def snowflake_status(user=Depends(verify_token),
...
@app.get("/api/auth/snowflake-auth-url")
async def get_snowflake_auth_url(user=Depends(verify_token),
                                 credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Trigger a Gateway tool call to get the 3LO auth URL (-32042 elicitation)."""
    ...
    payload = json.dumps({
        "jsonrpc": "2.0", "id": 1, "method": "tools/call",
        "params": {"name": "SnowflakeMCPServer3LO___credit-risk-analyst", "arguments": {"message": "test"}},
    }).encode()
    req = urllib.request.Request(gw_url, data=payload, headers={
        "Authorization": f"Bearer {user_token}",
        "Content-Type": "application/json",
...
@app.post("/api/auth/complete-snowflake-auth")
async def complete_snowflake_auth(req: SnowflakeAuthRequest, user=Depends(verify_token),
                                  credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Complete the 3LO session binding after Snowflake consent redirect."""
    try:
...
def _extract_elicitation_url(raw_response):
    ...
    try:
        import re
        # Match AgentCore Identity OAuth authorize URL or Snowflake OAuth URL
        url_match = re.search(r'(https://[^\s"<>]+oauth2/authorize[^\s"<>]*)', text)
        if url_match:
            return url_match.group(1)
        url_match = re.search(r'(https://[^\s"<>]+snowflakecomputing\.com/oauth/authorize[^\s"<>]*)', text)
        if url_match:
            return url_match.group(1)
    except Exception:
        pass
...
async def chat(req: ChatRequest, user=Depends(verify_token),
    ...
    async def run_agent():
        ...
        try:
            ...
            if auth_url:
                _jobs[job_id] = {
                    "status": "snowflake_auth_required",
...
async def chat_status(job_id: str, user=Depends(verify_token)):
    ...
    # 3LO: Return auth_url for snowflake_auth_required status
    if job["status"] == "snowflake_auth_required":
        return {"status": "snowflake_auth_required", "auth_url": job["result"]["auth_url"]}
