...
class FoundationStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs):
        ...
        prefix = self.node.try_get_context("project_prefix") or "CreditRisk3LO"
        ...
        # 3LO requires: GetWorkloadAccessToken, GetResourceOauth2Token,
        # CompleteResourceTokenAuth, secretsmanager:GetSecretValue
        # Using broad bedrock-agentcore:* for demo simplicity (covers all 3LO actions)
            ...
            inline_policies={
                "GatewayPolicy": iam.PolicyDocument(
                    statements=[
                        ...
                        # Broad AgentCore permissions — covers Gateway + Identity (3LO) actions:
                        # GetWorkloadAccessToken, GetWorkloadAccessTokenForUserId,
                        # GetResourceOauth2Token, CompleteResourceTokenAuth, InvokeGateway
                        iam.PolicyStatement(
        ...
        # --- ECS Task Role ---
        # 3LO: backend needs CompleteResourceTokenAuth for session binding after consent
            ...
            inline_policies={
                "EcsTaskPolicy": iam.PolicyDocument(
                    statements=[
                        ...
                        # 3LO: backend calls CompleteResourceTokenAuth after user consent
                        iam.PolicyStatement(
                            actions=[
                        ...
                        # 3LO: CompleteResourceTokenAuth internally reads OAuth client secret
                        iam.PolicyStatement(
                            actions=["secretsmanager:GetSecretValue"],
                            resources=[
                                f"arn:aws:secretsmanager:{self.region}:{self.account}:secret:{prefix_lower}*",
