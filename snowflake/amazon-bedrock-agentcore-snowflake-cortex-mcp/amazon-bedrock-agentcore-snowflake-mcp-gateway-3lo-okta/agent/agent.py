"""Credit Risk Agent — Bedrock KB + Snowflake MCP Server (3LO + Okta).
...
Auth: Cognito JWT (inbound to gateway) + Okta 3LO OAuth (outbound via AgentCore Identity).
Snowflake trusts Okta-issued JWTs via External OAuth — per-user RBAC on each query.
"""
import base64
import json
import os
...
TARGET_NAME = os.environ.get("TARGET_NAME", "SnowflakeMCPServer3LOOkta")
...
def _call_mcp_tool(tool_name, arguments):
    ...
    # Check for -32042 URL elicitation (3LO — user needs to authenticate via Okta)
    if "error" in parsed and parsed["error"].get("code") == -32042:
        ...
        return {"error": f"-32042: Okta authentication required", "elicitation_url": urls[0] if urls else "", "code": -32042}
...
Okta SSO login redirect. Do NOT retry the tool call — just report the error as-is.
