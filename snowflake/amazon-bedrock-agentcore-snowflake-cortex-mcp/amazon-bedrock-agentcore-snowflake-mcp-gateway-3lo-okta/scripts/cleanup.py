"""Cleanup all resources deployed by the 3LO + Okta Credit Risk Agent project.
...
4. Snowflake objects (DB_3LO_OKTA, MCP Server, ANALYST_ROLE, AGENTCORE_3LO_OKTA_INT)

ISOLATION: Only drops objects specific to this project. Will NOT touch:
- CREDIT_RISK_DB (2LO project)
- CREDIT_RISK_DB_3LO (vanilla 3LO project)
- AGENTCORE_GATEWAY_OAUTH (2LO Snowflake built-in OAuth)
- agentcore_okta_ext_oauth (2LO External OAuth)
- AGENTCORE_3LO_INT (vanilla 3LO Snowflake OAuth)
- CREDIT_RISK_WH (shared warehouse)
...
def cleanup_cdk(profile):
    """Destroy all CreditRisk3LOOkta-* CDK stacks."""
...
def cleanup_snowflake():
    ...
    # Isolation guard: refuse to run if this config points at 2LO or vanilla-3LO resources
    PROTECTED_DBS = {"CREDIT_RISK_DB", "CREDIT_RISK_DB_2LO", "CREDIT_RISK_DB_3LO"}
    if db in PROTECTED_DBS or not db.endswith("_3LO_OKTA"):
        print(f"  ⚠️  Refusing to clean up Snowflake: database '{db}' is not a 3LO+Okta database.")
        print(f"      This project only drops databases ending in _3LO_OKTA.")
        print(f"      To clean up manually: review snowflake_config.json and drop objects yourself.")
        return
    ...
    try:
        ...
        # Only drop 3LO+Okta-specific objects — do NOT touch 2LO or vanilla 3LO resources
        for sql in [
            f"DROP DATABASE IF EXISTS {db}",
            "DROP SECURITY INTEGRATION IF EXISTS AGENTCORE_3LO_OKTA_INT",
            # Do NOT drop ANALYST_ROLE — name-shared with vanilla 3LO project
            # Do NOT drop CREDIT_RISK_WH — shared with 2LO / 3LO
            # Do NOT drop AGENTCORE_GATEWAY_OAUTH, agentcore_okta_ext_oauth, AGENTCORE_3LO_INT — other projects
        ]:
            try:
...
def cleanup_local_configs(skip_snowflake=False):
    ...
    configs = [
        ...
        # Remove Okta config so next deploy forces re-entry of Okta credentials
        "okta_config.json",
    ]
    # Only remove Snowflake configs if not skipping Snowflake cleanup
    if not skip_snowflake:
        configs += [
            "snowflake_config.json", "snowflake_mcp_config.json",
        ]
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Clean up all 3LO+Okta project resources")
    parser.add_argument("--profile", default=os.environ.get("AWS_PROFILE"))
    parser.add_argument("--skip-snowflake", action="store_true", help="Skip Snowflake cleanup")
    args = parser.parse_args()

    print("=" * 60)
    print("Credit Risk Agent (3LO + Okta) — Full Cleanup")
    print("=" * 60)
