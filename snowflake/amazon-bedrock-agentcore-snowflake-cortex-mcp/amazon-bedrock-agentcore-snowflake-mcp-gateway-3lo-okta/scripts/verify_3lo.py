"""Verify per-user tracking: compare query history between 3LO+Okta, 3LO, and 2LO databases."""
import os
...
DB_3LO_OKTA = os.environ.get("SNOWFLAKE_DATABASE", "CREDIT_RISK_DB_3LO_OKTA")
DB_3LO = "CREDIT_RISK_DB_3LO"
...
for db, label in [(DB_3LO_OKTA, "3LO + Okta (Per-User via Okta SSO)"),
                  (DB_3LO, "3LO (Per-User via Snowflake built-in OAuth)"),
                  (DB_2LO, "2LO (Shared Service Account)")]:
    print(f"\n{'='*70}")
...
conn.close()
print(f"\n{'='*70}")
print("✅ 3LO+Okta: USER_NAME = YOUR Snowflake user (Okta sub → LOGIN_NAME mapping)")
print("   3LO:      USER_NAME = YOUR Snowflake user (built-in OAuth consent)")
print("   2LO:      USER_NAME = shared service account (e.g., AGENTCORE_OKTA_SERVICE_USER)")
