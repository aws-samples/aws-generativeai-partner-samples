...
try:
    ...
    agent_config_id = os.getenv("LD_AI_CONFIG_ID_GENERAL", "general-assistant")
    
    logger.info(f"Initializing general-assistant agent with role: {agent_role}")
    
    # Load AI Config prompt with fallback
    GENERAL_ASSISTANT_SYSTEM_PROMPT, _tracker, _config, _tools = load_ai_agent_prompt(
...
def general_assistant(query: str) -> str:
    ...
    try:
        ...
        logger.info("Loading model config: {}".format(_config.model.name))
        bedrock_model = BedrockModel(
            model_id= _config.model.name,
            temperature= _config.model.get_custom("temperature"),
            top_p= _config.model.get_custom("topP"),
            streaming=False,
        )
        ...
        general_agent = Agent(
            ...
            callback_handler=lambda **kwargs: event_loop_tracker(_tracker, logger, **kwargs),
