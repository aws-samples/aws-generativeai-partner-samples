...
try:
    ...
    COMPUTER_SCIENCE_ASSISTANT_SYSTEM_PROMPT, cs_ai_tracker, cs_ai_config = load_ai_config_prompt(
...
def computer_science_assistant(query: str) -> str:
    ...
    try:
        ...
        logger.info("Loading model config: {}".format(cs_ai_config.model.name))
        bedrock_model = BedrockModel(
            model_id= cs_ai_config.model.name,
            temperature= cs_ai_config.model.get_custom("temperature"),
            top_p= cs_ai_config.model.get_custom("topP"),
        )
        ...
        cs_agent = Agent(
            ...
            callback_handler=lambda **kwargs: event_loop_tracker(cs_ai_tracker, logger, **kwargs),
