...
try:
    ...
    ENGLISH_ASSISTANT_SYSTEM_PROMPT, english_ai_tracker, english_ai_config = load_ai_config_prompt(
    ...
    english_ai_tracker = None
    english_ai_config = None
...
def english_assistant(query: str) -> str:
    ...
    try:
        ...
        logger.info("Loading model config: {}".format(english_ai_config.model.name))
        bedrock_model = BedrockModel(
            model_id= english_ai_config.model.name,
            temperature= english_ai_config.model.get_custom("temperature"),
            top_p= english_ai_config.model.get_custom("topP"),
        )
        ...
        english_agent = Agent(
            ...
            callback_handler=lambda **kwargs: event_loop_tracker(english_ai_tracker, logger, **kwargs),
